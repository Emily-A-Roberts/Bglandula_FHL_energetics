...
Pick representative barnacles that are close to these sizes
...
#Convert from UCT to PST
min(datetime_int)
max(datetime_int)
which(datetime_int>=as.POSIXct("2018-08-01 00:00:00")&datetime_int<=as.POSIXct("2018-08-02 00:00:00"))
...
```{r}
#| label: 1-day timeseries1_physio
#| fig-width: 6.5
#| fig-height: 1.5
if(timing == 1){
...
```{r}
#| label: 1-day timeseries2_physio
#| fig-width: 6.5
#| fig-height: 1.5
...
```{r}
#| label: Predicted vs. observed growth
#| fig-width: 4
#| fig-height: 4
if(timing == 1){
  #setwd("~/Box/Sarah and Molly's Box/FHL data/plots/Interval 1")
...
Plot physiological rates over time
...
```{r}
  ...
  df_round <- df_round$modified_df
  

```

```{r}
#| label: Physio-timeseries
#| fig-width: 6.5
#| fig-height: 1.5

 n_len <- length(new_intake$Low)
...
```{r}
#| label: SFG-timeseries-cumulative-overlay
#| fig-width: 6.5
#| fig-height: 1.5
  par(mfrow = c(1,3))
  par(mar = c(1,1,1,1)+.1)
...
plot(new_intake$date, daily_SFG$Mid,type = "l", ylim = c(-1,4), yaxs = "i",
       col = "orange", xlab = "", ylab = "Physiological rate (J / day)", bty = "n",xaxt="n")
par(new=TRUE)  
plot(new_intake$date,new.joule_mass[2:189,2], col = "black",type = "l", ylim = c(-13,60),axes=FALSE,bty = "n",xaxt="n") #Low
  #axis(side = 4) 
 axis.Date(1, at=seq(min(df_round), max(df_round), by="2 mon"), format="%b")
...
plot(new_intake$date, daily_SFG$Upper,type = "l", ylim = c(-1,4), yaxs = "i",
       col = "orange", xlab = "", ylab = "Physiological rate (J / day)", bty = "n",xaxt="n")
par(new=TRUE)  
plot(new_intake$date,new.joule_mass[2:189,1], col = "black",type = "l", ylim = c(-13,60),axes=FALSE,bty = "n",xaxt="n") #Low
  axis(side = 4) 
   axis.Date(1, at=seq(min(df_round), max(df_round), by="2 mon"), format="%b")
...
plot(new_intake$date, daily_SFG$Low,type = "l", ylim = c(-1,4), yaxs = "i",
       col = "orange", xlab = "", ylab = "Physiological rate (J / day)", bty = "n",xaxt="n")
par(new=TRUE)  
plot(new_intake$date,new.joule_mass[2:189,3], col = "black",type = "l", ylim = c(-13,60),axes=FALSE,bty = "n",xaxt="n") #Low
  #axis(side = 4) 
 axis.Date(1, at=seq(min(df_round), max(df_round), by="2 mon"), format="%b")
...
```{r}
#| label: timeseries_temp_quantile
#| fig-width: 6.5
#| fig-height: 1.5
...
```{r}
#| label: timeseries_temp_quantile2
#| fig-width: 6.5
#| fig-height: 1.5
#pdf("temp_incl_3Q.pdf", width=6.5, height=1.5)
par(mfrow = c(1,3))
...
```{r}
#| label: timeseries_food_submergence_time
#| fig-width: 6.5
#| fig-height: 1.5
...
axis(side = 4, at = pretty(range(food.list[[1]])))      # Add second axis
mtext("ug / L Chl", side = 4, line = 3)  
 axis.Date(1, at=seq(min(df_round.2), max(df_round.2), by="2 mon"), format="%b")
